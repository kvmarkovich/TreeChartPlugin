getOrgChart = function (b, a) {
    ...
    this.localization = {'name':'Имя', 'position':'Должность', 'contacts':'Контакты'};
                    ...
                    switch (themeProperty) {
                        ...
                        case"text":
                            ...
                            for (k = 0; k < this.config.primaryColumns.length; k++) {
                                ...
                                f.push(this.theme.text.replace("[index]", g).replace("[text]", d.data[c]).replace("[y]", e.y).replace("[x]", e.x).replace("[rotate]", e.rotate).replace("[width]", e.width));
                                g++
                            }
                            break
                    }
...
getOrgChart.DETAILS_VIEW_INPUT_HTML = '<div data-field-name="[label]"><div class="get-label">[label]</div><div class="get-data">[value]</div></div>';
...
getOrgChart.prototype._zg = function (c) {
    ...
    for (label in c.data) {
        if(label != 'image')
            d += getOrgChart.DETAILS_VIEW_INPUT_HTML.replace(/\[label]/g, this.localization[label] ? this.localization[label] : label).replace("[value]", c.data[label]);
            h.push(label)
    }
    d += a(h, this._aG);
